# Milestone 5: Advanced Agent Interactions & Scheduling

**Goal**: Enable agent-to-agent conversations, group channel dynamics, the skills system, and autonomous scheduled behavior.

```mermaid
graph TD
    S50[S-5.0 Agent-to-agent chains] --> S52[S-5.2 Skills system]
    S51[S-5.1 Group channel dynamics] --> S52
    S52 --> S53[S-5.3 Scheduler]
```

---

## [S-5.0] Agent-to-Agent DM Chains
...
### Acceptance Criteria
- [ ] [AC-5.0.1] When Agent A DMs Agent B, a run is enqueued for Agent B to respond
- [ ] [AC-5.0.2] Chain depth tracked in run metadata, max 3 hops to prevent infinite loops
- [ ] [AC-5.0.3] Each agent in the chain sees the full DM history as context
- [ ] [AC-5.0.4] Unit tests for chain depth enforcement, integration test for a 2-agent exchange
- [ ] [AC-5.0.5] Sentry traces show the full chain with linked spans
...
## [S-5.1] Group Channel Response Ordering
...
### Acceptance Criteria
- [ ] [AC-5.1.1] Agents respond one at a time (not in parallel) to avoid message ordering issues
- [ ] [AC-5.1.2] Random delay (1-5s) between agent invocations for natural feel
- [ ] [AC-5.1.3] Each agent sees previous agent responses in their conversation context
- [ ] [AC-5.1.4] Unit test for sequential ordering logic
- [ ] [AC-5.1.5] Sentry traces show sequential processing with delays
...
### Acceptance Criteria
- [ ] [AC-5.2.1] Each skill file has YAML frontmatter (`name`, `description`) + markdown body
- [ ] [AC-5.2.2] `loadSkill()` reads and returns skill content
- [ ] [AC-5.2.3] `listSkills()` discovers all skills from `.skills/` directory
- [ ] [AC-5.2.4] Skills can be injected into agent prompts on demand
- [ ] [AC-5.2.5] Unit tests for skill loader
- [ ] [AC-5.2.6] Sentry span for skill loading
...
### Acceptance Criteria
- [ ] [AC-5.3.1] Scheduler loop runs every 10s, picks up due messages
- [ ] [AC-5.3.2] Fired messages marked as `'fired'` to prevent re-execution
- [ ] [AC-5.3.3] `instrumentation.ts` starts scheduler on Node.js runtime only
- [ ] [AC-5.3.4] Rate limit: max 1 scheduled fire per agent per 5 minutes
- [ ] [AC-5.3.5] Unit tests for scheduler logic, integration test for fire-and-mark flow
- [ ] [AC-5.3.6] Sentry spans for scheduler loop iterations and fires

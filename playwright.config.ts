...
// run-e2e.js passes E2E_OUTPUT_DIR so traces land next to server.log.
// Fallback timestamp for standalone `npx playwright test` runs.
const outputDir =
  process.env.E2E_OUTPUT_DIR ??
  `test-results/${new Date().toISOString().replace(/[:.]/g, "-").replace("T", "_").slice(0, 19)}`;

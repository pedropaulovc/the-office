 ...
 */
export function getAllUnreads(): Promise<Record<string, Record<string, number>>> {
  return withSpan("getAllUnreads", "db.query", async () => {
    const rows = await db
      .select({
 ...
 */
export function getUnreadsByUser(userId: string): Promise<Record<string, number>> {
  return withSpan("getUnreadsByUser", "db.query", async () => {
    const rows = await db
      .select({
        channelId: channelReads.channelId,
        unreadCount: sql<number>`count(${messages.id})::int`.as("unread_count"),
 ...
 */
export function markChannelRead(userId: string, channelId: string): Promise<void> {
  return withSpan("markChannelRead", "db.query", () =>
    db
      .insert(channelReads)
